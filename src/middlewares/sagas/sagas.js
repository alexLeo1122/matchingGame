...
import { selectGamePaths, selectCardIds, setResultFailed, setResultSucceed, setBlockedPaths, setGameContinue } from "../../features/game-contents/gamecontents.slice";
import { getBlockedPaths, PathObj, Status, findShortestPath,checkRange,nextUp, nextDown, nextLeft, nextRight, board, Game_, Game_Board} from "../../utils/shortTestPath";
import { SquareCons, delay, shuffleArr } from "../../utils/func.utils";
import { Square_Visibility } from "../../features/square/square.component";
import { selectGameHintPath, selectRemainingCardIds, setGameHint, setGameSolutionsFalse, setGameSolutionsTrue, setRemainingCardIds } from "../../features/game-solutions/game-solutions.slice";
  ...
  //  remainingCardIds
   ...
   const x2remainingCardIds = []; 
    remainingCardIds.forEach(ele=>{
      x2remainingCardIds.push(ele); x2remainingCardIds.push(ele);
    })
   shuffleArr(x2remainingCardIds);
   const newCardsObjMap = {}
   stockedPaths.forEach((boardId,index) => {
    newCardsObjMap[boardId]={}; newCardsObjMap[boardId]["cardId"] = x2remainingCardIds[index];
});
...
export function* toggleHintModeWorker() {
 ...
 if (storeHint){yield put(setIsHintModeFalse())};
}
